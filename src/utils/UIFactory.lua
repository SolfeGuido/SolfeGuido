...
function UIFactory.createIconButton(container, config)
	...
	return addToState(
		config,
		container:addEntity(IconButton, {
...
function UIFactory.createTextButton(container, config)
	...
	local btnText = love.graphics.newTextBatch(config.font, tr(config.text))
	if config.icon then
		config.icon = love.graphics.newTextBatch(
	...
	return addToState(
		config,
		container:addEntity(TextButton, {
			text = btnText,
			icon = config.icon,
			x = config.x,
			y = config.y,
...
function UIFactory.createTitle(container, config)
	if type(config.text) == "string" then
		...
		config.text = love.graphics.newTextBatch(
			config.fontSize and assets.fonts[config.fontName](config.fontSize) or config.font,
			tr(config.text)
		)
	end
...
function UIFactory.createRadioButton(container, config)
	...
	if config.icon and not config.image then
		config.image =
			love.graphics.newTextBatch(assets.fonts.Icons(config.size or Vars.titleSize), assets.IconName[config.icon])
	elseif config.text and not config.image then
		local font = config.font or "MarckScript"
		config.image = love.graphics.newTextBatch(assets.fonts[font](config.size or Vars.titleSize), tr(config.text))
	end
	return addToState(
		config,
