...
function Measure:new(container, options)
	...
	self.image = love.graphics.newTextBatch(font, assets.IconName[self.keyData.icon])
	...
	self.noteIcon = love.graphics.newTextBatch(font, assets.IconName.QuarterNote)
	self.wrongNoteIcon = love.graphics.newTextBatch(font, assets.IconName.GhostNote)
	self.noteChoice = 1
...
function Measure:draw()
	...
	love.graphics.draw(self.image, self.x + self.keyPadding, self.y + self.keyData.yOrigin * self.noteHeight)
...
function Measure:getNoteName(noteName)
	...
	if Config.noteStyle == "englishNotes" then
		return sub
	end
