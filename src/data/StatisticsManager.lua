...
local function fixStatistics(games)
	...
	table.sort(games, function(a, b)
		return os.difftime(os.time(a.date), os.time(b.date)) < 0
	end)
	local tPlayed = 0
	local tCorrect = 0
	local tWrong = 0
...
function StatisticsManager.init()
	...
	gameList = data.games or default.games
	globalStats = data.globals or default.globals
end
...
function StatisticsManager.add(stats)
	...
	if obj.correctNotes > 0 then
		globalStats.avgReacTime = ((globalStats.avgReacTime * tCorrect) + (obj.avgReacTime * obj.correctNotes))
