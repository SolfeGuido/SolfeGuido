...
function ScoreboardState:init()
	...
	self.texts = {}
	self.titles = {}
	local title = love.graphics.newTextBatch(assets.fonts.MarckScript(Vars.titleSize), tr("scoreboard"))
	...
	for _, v in ipairs(Vars.userPreferences.keySelect) do
		local text = love.graphics.newTextBatch(font, tr(v))
	...
	for i, v in ipairs(levels) do
		local text = love.graphics.newTextBatch(font, v == "all" and tr(v) or tr("level", { level = v }))
		...
		for _, key in ipairs(Vars.userPreferences.keySelect) do
			...
			text = love.graphics.newTextBatch(font, tostring(score))
	...
	for i, v in ipairs(Vars.userPreferences.time) do
		elements[#elements + 1] = {
			element = UIFactory.createRadioButton(self.radioButtons, {
				...
				image = love.graphics.newTextBatch(font, v),
			}),
