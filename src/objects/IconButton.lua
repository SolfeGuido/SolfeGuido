...
function IconButton:new(container, config)
	...
	self.image = love.graphics.newTextBatch(defaultFont, config.icon)
...
function IconButton:contains(x, y)
	return self.x <= x and (self.x + self._width) >= x and self.y <= y and (self.y + self.height) >= y
end
...
function IconButton:draw()
	...
	love.graphics.draw(
		self.image,
		lume.round(self.x + self.xOrigin + self.padding),
		lume.round(self.y + self.yOrigin + self.padding),
		self.rotation,
		nil,
		nil,
		lume.round(self.xOrigin),
		lume.round(self.yOrigin)
	)
end
