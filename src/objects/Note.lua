...
function Note:wrong()
	...
	self.name = love.graphics.newTextBatch(font, self.container:getNoteName(self.note))
...
function Note:fadeTo(color)
	self.timer:tween(Vars.note.fadeAway, self, { color = color }, "linear", function()
		self.isDead = true
	end)
end
